//Definire una struct “tipolibro” con gli attributi: titolo (string), autore (string), anno di edizione (int) e prezzo (int), dove “titolo” e “autore” sono stringhe senza spazi. 
...
int main() {
		...
		for (i = 0; i<N; i++)
		{
		 ...
		 cout << libri[i].titolo << " " << libri[i].autore << " " << libri[i].anno << " " << libri[i].prezzo << endl;
