...
class BST 
  ...
  void rand_insertR(link& h, Item x)
   ...
   void stampa_risultatiR(link& h, BST<Item, Key>& b)
   {
	   if (h == 0) return;

	   Item it = b.search(h->item.key());

	   float media = ((float)h->item.getvoto() + (float) it.getvoto())/2;
	   ...
	   stampa_risultatiR(h->l, b);
	   stampa_risultatiR(h->r, b);
   }
	public:
      BST()
        { head = 0; }
	  ...
	  void balance()
		{ balanceR(head);}
	  void stampa_risultati(BST<Item, Key>& b)
	  {
		  studentiA = 0;
		  studentiB = 0;
		  stampa_risultatiR(head,b);
		  cout << endl;
		  cout << "media voti sufficiente parteA: " << mediaA / studentiA << endl;
		  cout << "media voti sufficiente parteB: " << mediaB / studentiB << endl;
	  }
  };
