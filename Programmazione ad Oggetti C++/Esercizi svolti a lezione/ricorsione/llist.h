...
public:
  ...
  int count_rec(Link<E>* elem)
  {
	  if (elem->next == NULL) return 0;
	  return 1 + count_rec(elem->next);
  }

  //esempi di alcune funzioni ricorsive per liste singolarmente concatenate
  
  // conta il numero di elementi nella lista ricorsivamente
  int count()
  {
	  return count_rec(head);
  }

  // funzione di visita di esempio
  void visit(Link<E>* elem)
  {
	  cout << elem->element.key() * 2 << endl;
  }
  ...
  // attraversa ricorsivamente la lista eseguendo la funzione visit per ogni elemento
  void traverse()
  {
	  traverse_rec(head);
  }
